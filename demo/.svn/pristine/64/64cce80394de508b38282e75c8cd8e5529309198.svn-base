...
* Purpose:
*********************************************************************/
#ifndef KSEARCHFILE_H
#define KSEARCHFILE_H
#include <vector>
#include <string>
#include <io.h>
#include <cassert>
...
{
public:
// 	KSearchFile():m_nNextEnvDirId(0)
// 	{
// 	}

	KSearchFile(const TCHAR* pszFileName):m_nNextEnvDirId(0)
	{
		if (pszFileName)
		...
		*/
		...
		kxe_get_scom_reg_path(strDllRegPath);
...
// 	}
	...
	int SearchInReg(const TCHAR* szFileFullName, size_t size)
	{
		assert(!m_strDllRegPath.empty());	
		if (m_strDllRegPath.empty())
		{
			return E_SCOM_DLL_PATH_NOT_ACCQUIRED_FROM_CONFIGURATION_FILE;
		}
...
// 			);
		...
		nRetCode = RegSetValueEx(
			...
			REG_SZ,
			(BYTE*)szFileFullName,
			(DWORD)(_tcslen(szFileFullName) * sizeof(TCHAR))
			);
