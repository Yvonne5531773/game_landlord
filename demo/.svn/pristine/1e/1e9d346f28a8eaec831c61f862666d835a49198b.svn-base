...
inline HRESULT kxe_get_config_service_name_form_cfgfile(const wchar_t* pszFile, const wchar_t* pszBaseServiceName, std::wstring& strConfigServiceName)
{
	...
	if (!_kxe_check_is_file_exist(pszFile))
		return E_FAIL;
...
static inline const wchar_t* kxe_get_last_slash_substr_ptrw(const wchar_t* pszPathName)
{
	const wchar_t* pszShortFileName = NULL;
	if (pszPathName)
	{
		pszShortFileName = wcsrchr(pszPathName, L'\\');
		if (NULL == pszShortFileName)
			pszShortFileName = pszPathName;
		else
			++pszShortFileName;
	}
	return pszShortFileName;
}

//////////////////////////////////////////////////////////////////////////
//获取进程目录下一个特殊的配置文件路径
inline void kxe_get_process_special_cfgfile(std::wstring& strConfigFilePath)
{
	std::wstring strPath;
	std::wstring strImageFileName;
	std::wstring strCfgFileName;
	wchar_t* pExtPos = NULL;
	kxe_get_load_module_path(NULL, strPath);

	strImageFileName = kxe_get_last_slash_substr_ptrw(strPath.c_str());

	pExtPos = wcsrchr(&(strImageFileName[0]), L'.');
	if (pExtPos != NULL)
	{
		*pExtPos = 0;
	}
	strCfgFileName = &(strImageFileName[0]);
	strCfgFileName += L"_sp.xcf";

	::PathRemoveFileSpecW(&(strPath[0]));
	strConfigFilePath = &(strPath[0]);
	strConfigFilePath += L"\\";
	strConfigFilePath += strCfgFileName;
}
...
inline HRESULT kxe_get_config_service_name(const wchar_t* pszBaseServiceName, std::wstring& strConfigServiceName)
{
	...
	HRESULT hr = E_FAIL;
	std::wstring strConfigFilePath;
	kxe_get_process_special_cfgfile(strConfigFilePath);
